  ...
  <div>
    <div v-if="!showQuestions" class="host-home card">
      <div v-if="!isRoomReady" class="card-body">
        ...
        <template v-if="quizType == 'CustomQuiz'">
          <div>
            ...
            <div class="sheets-template">
              <a class="orange-txt" :href="templateCopyURL" target="_blank" role="button">
              ...
              <p class="card-text template-instructions">
                ...
                <input class="form-control input-box" placeholder="Add the URL to your sheet" v-model="sheetURL"
        ...
        <input class="form-control input-box" placeholder="Enter nickname" v-model="hostNickname"
          :disabled="createBtnClicked" @keyup.enter="createQuizRoom()" />

        <button type="button create-random-btn" class="btn" @click="createQuizRoom()" :disabled="createBtnClicked">
          {{ btnText }}
        </button>
        <div class="alert alert-danger sheet-error" role="alert" v-if="sheetURLErr">
    ...
    <div v-if="showQuestions && !showFinalScreen" class="d-flex bd-highlight">
      <div class="question-flex bd-highlight">
        <Question :newQuestion="newQuestion" :newChoices="newChoices" :newQuestionNumber="newQuestionNumber"
          :isLastQuestion="isLastQuestion" :questionTimer="questionTimer" :correctAnswerIndex="correctAnswerIndex"
          :showImg="showImg" :questionImgLink="questionImgLink" :isAdminView="true"
          :correctAnswer="newChoices[correctAnswerIndex]" :showAnswer="showAnswer"></Question>
      </div>
      <div class="stats-flex bd-highlight">
        <LiveStats :numAnswered="numAnswered" :numPlaying="numPlaying" v-if="!showAnswer"></LiveStats>
        <div v-if="showAnswer">
          <Leaderboard :leaderboard="leaderboard" :isPlayer="false" :finalScreen="false"></Leaderboard>
          <AdminPanel :hostAdminCh="hostAdminCh" :prevQuestionNumber="newQuestionNumber" @end-quiz-now="endQuizNow()">
          </AdminPanel>
        </div>
      </div>
    </div>
    <div v-if="showFinalScreen" class="quizEnded">
      ...
      <Leaderboard :leaderboard="leaderboard" :isPlayer="false" :finalScreen="true"></Leaderboard>
    </div>
  </div>
</template>
